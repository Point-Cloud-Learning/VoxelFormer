...
class Semantic_Seg_Head(nn.Module):
    def __init__(self, num_category, dims, drop_rate):
        super(Semantic_Seg_Head, self).__init__()
...
class Voxel_Former_Semantic_Seg(nn.Module):
    def __init__(self, point_embedding_type, init_dim, dims, radius, query_num, scalar_attention_num, position_encode_type, voxel_size, quant_size, offset_attention_num, num_category,
                 drop_rate):
        super(Voxel_Former_Semantic_Seg, self).__init__()
        ...
        self.semantic_seg_head = Semantic_Seg_Head(num_category=num_category, dims=dims, drop_rate=drop_rate)
    ...
    def forward(self, inputs, coordinates, distance_matrices, groups, effective_groups):
        ...
        recognition = self.semantic_seg_head(locality_compensation)

        return recognition


def get_semantic_seg(init_dim, dims, voxel_size, query_num=60, scalar_attention_num=4, point_embedding_type="Semantic-based", position_encode_type="Contextual-based", quant_size=0.02,
                     offset_attention_num=4, num_category=40, drop_rate=0.5):

    radius = voxel_size / 2

    return Voxel_Former_Semantic_Seg(point_embedding_type, init_dim, dims, radius, query_num, scalar_attention_num, position_encode_type, voxel_size, quant_size, offset_attention_num,
