...
class Scalar_Attention(nn.Module):
    ...
    def forward(self, features, voxel_distances):
        ...
        if self.position_encode_type == "Contextual-based":
            ...
            position_bias = position_bias_query + position_bias_key
...
class Point_Voxel_Block(nn.Module):
    ...
    def forward(self, inputs):
        ...
        attention = self.trans(torch.matmul(attention_map, k_v))

        return attention, voxel_feature_set
...
class Locality_Compensation(nn.Module):
    ...
    def forward(self, point_features, compensation_features):
        return self.trans(torch.cat((point_features, compensation_features), dim=-1))


class Recognition(nn.Module):
    def __init__(self, num_category, dims, drop_rate):
        super(Recognition, self).__init__()
        self.head = nn.Sequential(
            nn.Dropout(drop_rate),
            nn.Linear(dims, 64),
            nn.ReLU(inplace=True),
            nn.Linear(64, num_category)
        )

    def forward(self, inputs):
        return self.head(torch.max(inputs, dim=1).values)
...
class Voxel_Former_Recognition(nn.Module):
    def __init__(self, point_embedding_type, init_dim, dims, radius, query_num, scalar_attention_num, position_encode_type, voxel_size, quant_size, offset_attention_num, num_category,
                 drop_rate):
        super(Voxel_Former_Recognition, self).__init__()
        ...
        self.locality_compensation = Locality_Compensation(dims=dims)
        self.recognition = Recognition(num_category=num_category, dims=dims, drop_rate=drop_rate)

        self.apply(_init_vit_weights)

    def forward(self, inputs, coordinates, distance_matrices, groups, effective_groups):
        ...
        locality_compensation = self.locality_compensation(point_voxel_attention, for_retrieve)
        recognition = self.recognition(locality_compensation)

        return recognition


def get_recognition(init_dim, dims, voxel_size, query_num=50, scalar_attention_num=3, point_embedding_type="Semantic-based", position_encode_type="Contextual-based", quant_size=0.02,
                    offset_attention_num=3, num_category=40, drop_rate=0.5):

    radius = voxel_size / 2

    return Voxel_Former_Recognition(point_embedding_type, init_dim, dims, radius, query_num, scalar_attention_num, position_encode_type, voxel_size, quant_size, offset_attention_num,
                                    num_category, drop_rate)
